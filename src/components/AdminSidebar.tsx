...
export const AdminSidebar: React.FC<AdminSidebarProps> = ({
  ...
  return (
    <>
      ...
      {/* Slide-over Drawer (Mobile) & Sticky Left Sidebar (Desktop) */}
      <aside 
        className={`fixed md:sticky top-0 inset-y-0 left-0 z-50 md:z-30 h-screen bg-white dark:bg-slate-900 border-r border-slate-200 dark:border-slate-800 text-slate-800 dark:text-slate-200 flex flex-col justify-between p-3 shadow-xl md:shadow-sm select-none overflow-y-auto overflow-x-hidden transition-all duration-300 ease-in-out ${
      ...
      >
        ...
        <div className="space-y-4">
          ...
          {/* Top Header & Collapse / Close Toggle Button */}
          <div className="flex items-center justify-between gap-1">
            <div 
              onClick={() => handleNavClick('dashboard')}
              className={`flex items-center gap-2 cursor-pointer p-1 rounded-xl hover:bg-slate-100 dark:hover:bg-slate-800 transition ${isCollapsed ? 'md:justify-center md:w-full' : ''}`}
              title="ERA INSPIRASI CMS"
            >
              <span className="bg-rose-600 text-white font-black text-sm px-2.5 py-1 rounded-xl shadow-md shadow-rose-600/30 tracking-wider shrink-0">
                ERA
              </span>
            ...
            <button
              onClick={onCloseMobile}
              className="md:hidden p-2 rounded-xl bg-slate-100 dark:bg-slate-800 text-slate-500 hover:text-slate-900 dark:hover:text-white font-bold transition"
            ...
            {!isCollapsed && (
              <button
                ...
                className="hidden md:block p-1.5 rounded-lg hover:bg-slate-100 dark:hover:bg-slate-800 text-slate-400 hover:text-slate-600 transition"
          ...
          {isCollapsed && (
            <button
              ...
              className="hidden md:flex w-full py-1.5 rounded-xl bg-slate-100 dark:bg-slate-800 hover:bg-rose-50 hover:text-rose-600 text-slate-500 dark:text-slate-400 text-xs items-center justify-center transition"
          ...
          {/* Admin Profile Card */}
          <div className={`bg-rose-50/60 dark:bg-slate-800 p-2 rounded-2xl border border-rose-100 dark:border-slate-700 flex items-center gap-2 ${isCollapsed ? 'md:justify-center' : ''}`}>
            <img
              src={user.avatar}
              alt={user.name}
          ...
          <div className="space-y-1">
            ...
            {mainNavItems.map((item) => {
              ...
              return (
                <button
                  ...
                  title={item.label}
                  className={`w-full ${(isCollapsed && !isMobileOpen) ? 'p-2.5 justify-center' : 'px-3 py-2.5 justify-between'} rounded-2xl text-xs font-extrabold transition-all flex items-center group active:scale-95 ${
                    isActive
                      ? 'bg-rose-600 text-white shadow-lg shadow-rose-600/25'
                      : 'text-slate-700 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800 hover:text-slate-900 dark:hover:text-white'
                  }`}
                >
                  <div className={`flex items-center ${(isCollapsed && !isMobileOpen) ? 'justify-center' : 'gap-2.5'} truncate`}>
            ...
            <div className="pt-2">
              {(!isCollapsed || isMobileOpen) ? (
                ...
                /* Collapsed Sidebar View for Settings Submenus on Desktop */
            ...
            <div className="pt-2 border-t border-slate-200 dark:border-slate-800">
              <button
                ...
                className={`w-full ${(isCollapsed && !isMobileOpen) ? 'p-2.5 justify-center' : 'px-3 py-2.5 justify-between'} rounded-2xl text-xs font-extrabold transition-all flex items-center group active:scale-95 bg-rose-50 dark:bg-rose-950/40 text-rose-700 dark:text-rose-300 hover:bg-rose-100 border border-rose-200/60 dark:border-rose-900/40`}
              >
                <div className={`flex items-center ${(isCollapsed && !isMobileOpen) ? 'justify-center' : 'gap-2.5'} truncate`}>
                  <Newspaper className="w-4 h-4 shrink-0 text-rose-600" />
                  {(!isCollapsed || isMobileOpen) && <span className="truncate">Lihat Portal Publik</span>}
                </div>
                {(!isCollapsed || isMobileOpen) && (
        ...
        <div className="pt-3 border-t border-slate-200 dark:border-slate-800 space-y-2">
          ...
          <button
            ...
            className={`w-full ${(isCollapsed && !isMobileOpen) ? 'p-2 justify-center' : 'px-3 py-2 justify-between'} rounded-xl bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-300 text-xs font-bold transition flex items-center border border-slate-200 dark:border-slate-700`}
          >
            <span className="flex items-center gap-2">
              {darkMode ? <Sun className="w-4 h-4 text-amber-500" /> : <Moon className="w-4 h-4 text-slate-600" />}
              {(!isCollapsed || isMobileOpen) && <span className="text-[11px]">{darkMode ? 'Mode Gelap' : 'Mode Terang'}</span>}
            </span>
            {(!isCollapsed || isMobileOpen) && <span className="text-[9px] text-slate-400 uppercase font-black">Ubah</span>}
          </button>
          ...
          <button
            ...
            title="Logout Admin"
            className={`w-full ${(isCollapsed && !isMobileOpen) ? 'p-2.5 justify-center' : 'px-3 py-2.5 justify-center gap-2'} rounded-2xl bg-rose-600 hover:bg-rose-500 text-white text-xs font-extrabold shadow-md shadow-rose-950/60 transition flex items-center active:scale-95`}
          >
